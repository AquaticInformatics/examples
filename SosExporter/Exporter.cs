    ...
    public class Exporter
    {
            ...
            TimeSeriesDescription timeSeriesDescription)
        {
            ...
            if (existingSensor == null || deleteExistingSensor || clearExportedData)
            {
                ...
                {
                    period = InferPeriodFromRecentPoints(timeSeries);
        ...
        private ComputationPeriod InferPeriodFromRecentPoints(TimeSeriesDataServiceResponse timeSeries)
        {
            ...
            var duration = recentPoints.Last().Timestamp.DateTimeOffset -
                           recentPoints.First().Timestamp.DateTimeOffset;

            var averageDuration = TimeSpan.FromTicks(duration.Ticks / recentPoints.Count);

            if (averageDuration < CommonPeriods.First().TimeSpan)
                return ComputationPeriod.Unknown;

            if (averageDuration > CommonPeriods.Last().TimeSpan)
                return CommonPeriods.Last().Period;

            return CommonPeriods.First(p => averageDuration < p.TimeSpan).Period;
        }
