...
{
    // Lovingly inspired from http://paulbourke.net/dataformats/hershey/
    public class VectorFont
    {
        public class Symbol
        {
            ...
            private int[] XyPoints { get; }

            private IReadOnlyList<Point> _points;

            public IReadOnlyList<Point> Points => RenderPoints();
            ...
            public Symbol(char character, int width, params int[] xypoints)
            {
                Character = character;
                Width = width;

                if (xypoints.Length % 2 != 0)
                    ThrowInvalidSymbolException($"Odd number of {nameof(xypoints)}: Count={xypoints.Length}");

                XyPoints = xypoints;
            }

            private void ThrowInvalidSymbolException(string message)
            {
                throw new ExpectedException($"Vector symbol '{Character}' (Ascii {(int)Character}): {message}");
            }

            private IReadOnlyList<Point> RenderPoints()
            {
                if (_points != null)
                    return _points;
                ...
                {
                    if (penPosition == null)
                        ThrowInvalidSymbolException("Unable to draw line because no pen position is known.");

                    // ReSharper disable once PossibleNullReferenceException
                    if (DoubleHelper.AreEqual(penPosition.X, point.X) && DoubleHelper.AreEqual(penPosition.Y, point.Y))
                        return;

                    var dX = Math.Abs(point.X - penPosition.X);
                    var dY = Math.Abs(point.Y - penPosition.Y);
                    ...
                    {
                        if (dY <= 0)
                            ThrowInvalidSymbolException($"Unable to render line with dX={dX} and dy={dY}");

                        steps = (int) dY;
                    }

                    dX = (point.X - penPosition.X) / steps;
                    dY = (point.Y - penPosition.Y) / steps;
                ...
                var pointPairs = new Queue<int>(XyPoints);
                const int penUpValue = -1;

                while (pointPairs.Any())
                {
                    var x = pointPairs.Dequeue();
                    var y = pointPairs.Dequeue();

                    if (x == penUpValue && y == penUpValue)
                    {
                        if (!isDrawingLine)
                            ThrowInvalidSymbolException("Extraneous PenUp command.");
        ...
        public static readonly IReadOnlyDictionary<char, Symbol> Symbols = new[]
            {
                ...
                new Symbol((char) 124, 8,
                    4, 25, 4, -7
                ),
                new Symbol((char) 125, 14,
