...
namespace SosExporter
{
    ...
    public static class DoubleHelper
    {
        ...
        public static bool AreSame(double? firstValue, double? secondValue)
        {
            if ((firstValue == null && secondValue == null) ||
                (firstValue.HasValue && secondValue.HasValue &&
                AreEqual(firstValue.Value, secondValue.Value)))
            {
                return true;
            }
        ...
        public static double NextAfter(double startValue, double towardsValue)
        {
            for (var i = 0; i < MaxUlps * 10; ++i)
            {
                startValue = nextAfterNative(startValue, towardsValue);
            }

            return startValue;
        }

        [DllImport("msvcrt.dll", EntryPoint = "_nextafter", ExactSpelling = true, CallingConvention = CallingConvention.Cdecl)]
        private static extern double nextAfterNative(double x, double y);
    }
}
